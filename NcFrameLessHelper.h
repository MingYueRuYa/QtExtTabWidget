...
{
public:
  using FilterEventCallBack = std::function<bool(QObject *, QEvent *)>;

public:
  explicit NcFramelessHelper( QObject* parent = 0 );
  ~NcFramelessHelper();

  void setFilterEventCallBack(FilterEventCallBack filterFunc);
  void activateOn( QWidget* topLevelWidget );
  void activeOnWithChildWidget( QWidget* topLevelWidget , QWidget *tagetWidget);
...
private:
  ...
  // add by Shixiong.Liu
  FilterEventCallBack filter_event_call_back_ = nullptr;
};
