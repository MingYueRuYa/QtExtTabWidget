...
void RoundShadowHelper::RoundShadow(QPainter* painter, const QRect& source_rect,
                                    RoundDirection dirc) {
  //不处理空数据或者错误数据
...
void RoundShadowHelper::FillRoundShadow(QPainter* painter, const QRect& rect,
                                        const QColor& fill_color, int radius) {
  painter->save();
  QPainterPath path;
...
void RoundShadowHelper::getGradient(const QPoint& start_point, const int radius,
