...
void RoundShadowHelper::RoundShadow(QPainter* painter,
  ...
  //获取图片尺寸
  int image_width = source_rect.width();
  int image_height = source_rect.height();

  // QPixmap new_pixmap = source_pixmap;
  // QPixmap dest_image(image_width + shadow_width_, image_height +
  // shadow_width_); dest_image.fill(Qt::transparent); QPainter
  // painter(&dest_image);
