...
QPixmap RoundImage::getRoundRectPixmap(QPixmap srcPixMap,
  ...
  //处理大尺寸的图片,保证图片显示区域完整
  QPixmap newPixMap = srcPixMap.scaled(
      imageWidth, (imageHeight == 0 ? imageWidth : imageHeight),
      Qt::IgnoreAspectRatio, Qt::SmoothTransformation);

  QPixmap destImage(imageWidth + 2 * kShadowWidth,
                    imageHeight + 2 * kShadowWidth);
  ...
  draw4BorderRectShadow(&painter, shadow_width, shadow_width2X,
                        destImage.size());
  draw4BorderArcShadow(&painter, shadow_width, shadow_width2X,
                       destImage.size());
  ...
  painter.drawPixmap(kShadowWidth, kShadowWidth, imageWidth, imageHeight,
                     newPixMap);
  return destImage;
}
...
void RoundImage::draw4BorderRectShadow(QPainter* painter,
                                       int shadow_width,
                                       int shadow_width2X,
...
void RoundImage::draw4BorderArcShadow(QPainter* painter,
                                      int shadow_width,
                                      int shadow_width2X,
                                      const QSize& image_size) {
  // 上左边
  ...
  myPath.arcTo(QRect(0, image_size.height() - shadow_width2X, shadow_width2X,
                     shadow_width2X),
               180, 90);
  ...
  myPath.arcTo(QRect(image_size.width() - shadow_width2X, 0, shadow_width2X,
                     shadow_width2X),
               0, 90);
  drawShadowArc(painter,
                QPoint(image_size.width() - shadow_width, shadow_width),
                QPoint(image_size.width(), 0), myPath);

  // 下右边
  myPath.moveTo(QPointF(image_size.width() - shadow_width,
                        image_size.height() - shadow_width));
  myPath.arcTo(QRect(image_size.width() - shadow_width2X,
