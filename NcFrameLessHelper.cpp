...
void NcWidgetData::updateRubberBandStatus()
{
  ...
  {
    if ( !mRubberBand )
        mRubberBand = new QRubberBand( QRubberBand::Line/*QRubberBand::Rectangle*/ );
  }
  else
  {
    delete mRubberBand;
...
bool NcWidgetData::cursorShapeChanged() const
{
  return mCursorShapeChanged;
}
...
void NcWidgetData::handleLeaveEvent( QEvent* /*event*/ )
{
  if ( !mLeftButtonPressed )
    mWidget->unsetCursor();
  mLeftButtonPressed = false;  //add by dragon
}
...
void NcFramelessHelper::setFilterEventCallBack(FilterEventCallBack filterFunc)
{
  filter_event_call_back_ = filterFunc;
}

bool NcFramelessHelper::eventFilter( QObject *obj, QEvent *event )
{
  if (d->mHashWidgetData.empty()) {
    return false;
  }  
     ...
     )
 {
   ...
   NcWidgetData* data = d->mHashWidgetData.begin().value();
   if (nullptr == data )
     return false;

   if (nullptr != filter_event_call_back_ && 
      filter_event_call_back_(obj, event) &&
      !data->cursorShapeChanged()) {
        return false;
   }
...
void NcFramelessHelper::activeOnWithChildWidget( QWidget* topLevelWidget , QWidget *tagetWidget)
{
  activateOn(topLevelWidget);
  tagetWidget->installEventFilter(this);
}
